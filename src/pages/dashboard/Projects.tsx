...
import { Folder, Play, MoreVertical, Loader2 } from 'lucide-react';
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <Link key={project.id} to={`/app/editor/${project.id}`}>
              <motion.div 
                whileHover={{ y: -5 }}
                className="bg-neutral-900/40 backdrop-blur-xl border border-white/5 rounded-[2.5rem] overflow-hidden hover:border-white/10 transition-all group cursor-pointer shadow-2xl h-full"
              >
                ...
                <div className="p-8">
                  <div className="flex items-start justify-between gap-6">
                    <div className="min-w-0">
                      <h3 className="font-black text-white mb-2 truncate tracking-tight text-lg group-hover:text-indigo-400 transition-colors">{project.title || 'Untitled Project'}</h3>
                      <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-neutral-500">
                        <span>{new Date(project.createdAt?._seconds * 1000 || Date.now()).toLocaleDateString()}</span>
                    ...
                    <Button variant="ghost" size="icon" className="shrink-0 -mr-3 text-neutral-600 hover:text-white hover:bg-white/5 rounded-2xl" onClick={e => e.preventDefault()}>
